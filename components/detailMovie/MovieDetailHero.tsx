...
const MovieDetailHero = ({ movieId, movieData }: MovieDetailHeroProps) => {
  ...
  const handleAddToWatchlist = async () => {
    const result = await addToWatchlistAction(movieData);
    if (result.success) {
      const watchlist = JSON.parse(localStorage.getItem('watchlist') || '{}');
    ...
    >
      ...
      <div className="w-full h-full bg-gray-900/50 flex justify-center items-center">
        <div className="max-w-[80%] lg:max-w-[90%] sm:max-w-full sm:px-3 sm:flex-col sm:items-center flex items-start gap-x-7">
          <Image priority width={250} height={400} className="rounded-md" src={`https://image.tmdb.org/t/p/original/${movieData?.poster_path}`} alt={movieData?.title} />
          <div className="flex-1 sm:mt-7">
            <h2 className="text-4xl font-bold text-white">{movieData?.title}</h2>
            ...
            <p className="text-[12px]  text-white mb-5">{movieData?.overview}</p>
